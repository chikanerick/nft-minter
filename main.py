...
if not web3.isConnected():
    raise Exception("Не удалось подключиться к сети Arbitrum")
...
def send_mint_transaction(wallet_private_key):
    try:
        ...
        calldata = method_id + web3.toHex(web3.toBytes(quantity)).lstrip("0x").zfill(64)
        ...
        print(f"Отправлена транзакция. Хэш: {web3.toHex(tx_hash)}")
        ...
        print(f"Ошибка при отправке транзакции с кошелька {wallet_address}: {e}")
        return False 
